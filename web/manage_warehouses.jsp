			...
			<div class="row-fluid">
				...
				<div class="box-content">
					...
					<div class="box span12">
					    ...
					    <div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable" id="warehouses" >
						    <thead>
							<tr>
							    <th>Name</th>
							    <th>Location</th>
							...
							<% int counter = 0;
							    for (Warehouse wh : WhComm.getWh(null)) {
								counter++;
								String button_id = Integer.toString(counter);
								String form_id = "form" + button_id;%>
							<tr>
							    <td><%=wh.getName()%></td>
							    <td class="center"><%=wh.getLocation()%></td>
							    <td class="center"><%=wh.getDescription()%></td>
							    <%	 if (wh.isHas_products()) {
								    out.println("<td class=\"center\"> Yes </td>");
								} else {
								    out.println("<td class=\"center\"> No </td>");
								}
							    %> 
							    <%	 if (wh.isIsOpen()) {
								    out.println("<td class=\"center\"> Yes </td>");
							    ...
							    %>
							    <td class="center">
								<form style='display:inline' action="wh_view" method="post">
								    <input type="hidden" name="wh_name" value="<%=wh.getName()%>" />
								    <%if (Integer.parseInt(wh_perm) > 0) {
								...
								<form style='display:inline' action="edit_warehouse.jsp" method="post">
								    <input type="hidden" name="wh_name" value="<%=wh.getName()%>" />
								    <%if (Integer.parseInt(wh_perm) == 2) {
								...
								<form style='display:inline' action="wh_rem" id ="<%=form_id%>" method="post">
								    <input type="hidden" name="wh_name" value="<%=wh.getName()%>" />
								    <%if (Integer.parseInt(wh_perm) == 2) {
									    out.println("<button class=\"btn btn-danger ask\" id=\"" + button_id + "\" type=\"submit\" >"
										    + "<a style=\"color:#FFFFFF;\" > "
						...
						<div style="float:left;">
						    <a class="btn btn-danger" href="create_new_warehouse.jsp" style="padding:6px 10px 6px 10px;">
